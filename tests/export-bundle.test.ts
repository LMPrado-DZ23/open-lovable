...
import {exportProjectBundle,validateExportBundleManifest,verifyExportBundle} from '../lib/artifacts/export-bundle';
...
test('ExportBundle includes a versioned manifest and verifies source provenance',()=>{
 ...
 const manifest=validateExportBundleManifest(JSON.parse(strFromU8(files['manifest.json'])));
 assert.equal(manifest.manifestVersion,1);assert.equal(manifest.projectVersion,3);assert.equal(manifest.files.length,2);
 verifyExportBundle(snapshot,manifest);assert.equal(strFromU8(files['src/App.jsx']),snapshot.files['src/App.jsx']);
});
