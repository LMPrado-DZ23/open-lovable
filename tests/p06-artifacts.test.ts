...
test('P06 rejects symlinked storage paths and pre-existing corruption',()=>{const root=mkdtempSync(join(tmpdir(),'artifacts-hardening-'));try{const workspace=join(root,'workspace_a'),project=join(workspace,'project_a');const data=Buffer.from('source bytes'),store=new LocalArtifactStore(root);writeFileSync(join(root,'outside'),'bad');symlinkSync(join(root,'outside'),workspace);assert.throws(()=>store.put('workspace_a','project_a',data,'text/plain','source'),/private directory|regular/i);rmSync(workspace,{recursive:true,force:true});const ref=store.put('workspace_a','project_a',data,'text/plain','source');writeFileSync(join(project,ref.digest),'corrupt');assert.throws(()=>store.put('workspace_a','project_a',data,'text/plain','source'),/integrity/i);}finally{rmSync(root,{recursive:true,force:true});}});
