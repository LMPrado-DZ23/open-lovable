...
export class QuotaService {
 ...
 assertCanStore(usage:QuotaUsage,kind:keyof QuotaUsage,bytes:number,artifacts:number=1):void {if(!Number.isSafeInteger(bytes)||bytes<0||bytes>32*1024*1024)throw new ProjectError('Invalid artifact size',400);const total=Object.values(usage).reduce((sum,value)=>sum+value,0);if(total+bytes>this.limits.maxBytes||artifacts>this.limits.maxArtifacts)throw new ProjectError('Workspace artifact quota exceeded',413);if(!(kind in usage))throw new ProjectError('Unknown artifact kind',400);}
