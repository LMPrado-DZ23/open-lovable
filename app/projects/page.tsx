 ...
 return <main className="min-h-screen bg-[#f7f7f5] text-[#252520]">
  <div className="mx-auto max-w-[1200px] px-[20px] py-[28px] md:px-[36px]">
   ...
   <form onSubmit={create} className="mb-[40px] grid items-end gap-[16px] rounded-lg border border-[#deded6] bg-white p-[24px] md:grid-cols-[1.1fr_1fr_auto]">
    <div className="min-w-0"><label htmlFor="project-name" className="mb-[8px] block text-[13px] font-medium">Nome do projeto</label><input id="project-name" value={name} onChange={event=>setName(event.target.value)} maxLength={120} required placeholder="Ex.: Agenda da minha empresa" disabled={busy} className="w-full min-w-0 rounded-md border border-[#d0d0c7] bg-white px-[12px] py-[12px] text-[14px] focus:outline-none focus:ring-2 focus:ring-orange-600"/></div>
    <div className="min-w-0"><p className="mb-[8px] text-[13px] font-medium">Modelo inicial</p><AIModelSelect label="Modelo do projeto" value={model} onValueChange={setModel} disabled={busy}/></div>
    <button type="submit" disabled={busy||!name.trim()} className="rounded-md bg-[#272721] px-[24px] py-[13px] text-[14px] font-medium text-white disabled:opacity-40">{busy?'Criando…':'Criar projeto'}</button>
   </form>
   ...
   <p className="mt-[36px] text-[12px] leading-relaxed text-[#77776e]">Workspace individual. Os dados ficam na instalação do servidor, não apenas nesta aba. Mantenha backup do diretório privado de dados.</p>
  </div>
 </main>;
}
